...
function refreshDashboard(){
  ...
  sh.getRange('A3:B14').setValues([
    ...
    ['LEGAL','=COUNTIF(PJU_Survey!G2:G,"LEGAL")'],
    ['ILEGAL','=COUNTIF(PJU_Survey!G2:G,"ILEGAL")'],
    ['Legal Tanpa IDPEL','=COUNTIFS(PJU_Survey!G2:G,"LEGAL",PJU_Survey!H2:H,"")'],
    ['Perlu Tindakan','=COUNTIF(PJU_Survey!X2:X,"Ya")'],
    ['Lampu Mati','=COUNTIF(PJU_Survey!T2:T,"Mati")'],
    ['Prioritas Tinggi/Darurat','=COUNTIF(PJU_Survey!Y2:Y,"Tinggi")+COUNTIF(PJU_Survey!Y2:Y,"Darurat")'],
    ['Data Wilayah','=COUNTA(PJU_Survey!I2:I)']
  ]);
...
function appendSurvey_(data){
  ...
  const legalitas=String(data['Status Legalitas']||'').trim().toUpperCase();
  ...
  sh.appendRow(row);
  return {ok:true,id,row:sh.getLastRow(),statusLegalitas:legalitas,idpel};
}
...
function getStats_(){
  ...
  const byLegalitas={'LEGAL':0,'ILEGAL':0};
  let perluTindakan=0,lampuMati=0,prioritasTinggi=0,legalTanpaIdpel=0;
  rows.forEach(r=>{
    if(Object.prototype.hasOwnProperty.call(byUlp,r['ULP']))byUlp[r['ULP']]++;
    if(Object.prototype.hasOwnProperty.call(byWilayah,r['Kabupaten/Kota']))byWilayah[r['Kabupaten/Kota']]++;
    const l=String(r['Status Legalitas']||'').toUpperCase();
    if(Object.prototype.hasOwnProperty.call(byLegalitas,l))byLegalitas[l]++;
    if(l==='LEGAL'&&!String(r['IDPEL']||'').trim())legalTanpaIdpel++;
...
function doGet(e){
  ...
  try{
    ...
    if(action==='config')return json_({ok:true,ulp:['ULP Sape','ULP Dompu','ULP Woha','ULP Bikot'],wilayah:['Kabupaten Bima','Kabupaten Dompu','Kota Bima'],legalitas:['LEGAL','ILEGAL']});
...
function doPost(e){
  try{
    ...
    if((body.action||'append')==='append')return json_({ok:true,result:appendSurvey_(body.data||body)});
